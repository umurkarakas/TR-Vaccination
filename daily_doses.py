...
for i in range(3):
    fig = go.Figure(data=[go.Table(
        header=dict(values=['Şehir', 'Toplam', '1.Doz', '2.Doz', '3.Doz', '18 Üstü 1.Doz', '18 Üstü 2.Doz'],
                    ...
                    align='left'),
        cells=dict(values=[data2.index[27*i:27*(i+1)], data2['toplam'][27*i:27*(i+1)], data2['1.doz'][27*i:27*(i+1)], data2['2.doz'][27*i:27*(i+1)], data2['3.doz'][27*i:27*(i+1)], data2["1dozoran"][27*i:27*(i+1)], data2["2dozoran"][27*i:27*(i+1)]],
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(values=['Şehir', 'Toplam', '1.Doz', '2.Doz', '3.Doz', '18 Üstü 1.Doz', '18 Üstü 2.Doz'],
                    fill_color = 'rgb(251,180,174)',
                    align='left'),
        cells=dict(values=[data2.sort_values(by=['toplamoran'], ascending = False).index[27*i:27*(i+1)], data2.sort_values(by=['toplamoran'], ascending = False)['toplam'][27*i:27*(i+1)], data2.sort_values(by=['toplamoran'], ascending = False)['1.doz'][27*i:27*(i+1)], data2.sort_values(by=['toplamoran'], ascending = False)['2.doz'][27*i:27*(i+1)], data2.sort_values(by=['toplam'], ascending = False)['3.doz'][27*i:27*(i+1)], data2.sort_values(by=['toplamoran'], ascending = False)["1dozoran"][27*i:27*(i+1)], data2.sort_values(by=['toplamoran'], ascending = False)["2dozoran"][27*i:27*(i+1)]],
                   fill_color = 'rgb(203,213,232)',
                   align = 'left'))])
    fig.add_annotation(x=0.007, y=1.03,
            text=date,
